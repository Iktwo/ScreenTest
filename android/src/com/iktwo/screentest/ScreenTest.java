...
import com.iktwo.screentest.R;

public class ScreenTest extends org.qtproject.qt5.android.bindings.QtActivity
{
    private static final String TAG = "ScreenTest";
    private static ScreenTest m_instance;
    ...
    public static void shareUrl(String name, String url)
    {
        ...
        if (!name.isEmpty()) {
            share.putExtra(Intent.EXTRA_TEXT, name + " - " + url + " via ScreenTest");
            m_instance.startActivity(Intent.createChooser(share, "Share " + name));
        } else {
            share.putExtra(Intent.EXTRA_TEXT, url + " via ScreenTest");
